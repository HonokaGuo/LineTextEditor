...
int main(void) {
	...
	while (1) {
		...
		switch (op) {
		case 'h':
			std::cout << "b:Display current file content f:Go to the first line l:Go to the last line p:Go to the previous line n:Go to the next line" << std::endl;
			std::cout << "a:Go to a specific line i:Insert a line d:Delete a line w:Replace a line t:Replace a substring z:Find a specific substring" << std::endl;
			std::cout << "c:Count the number of lines and characters in the file r:Reload the file s:Save the current file e:Exit the system" << std::endl;
			break;
		case 'b':
			display_all(pannel);
			break;//显示当前文件内容
		...
		case 'l':
			...
			//到最后一行
		case 'p':
			pannel.showLine(pannel.curLineNum - 1);
			break;
			//到前一行
		case 'n':
			pannel.showLine(pannel.curLineNum + 1);
